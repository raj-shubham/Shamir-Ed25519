...
var _ = Describe("Shamir Secret Sharing", func() {
	...
	Context("Sharing consistency (1)", func() {
		...
		Specify("any qualified subset can reconstruct the secret correctly", func() {
			indices := Ed25519RandomIndices(n)
			...
			for i := 0; i < trials; i++ {
				...
				Expect(Ed25519SharesAreConsistent(shares, k)).To(BeTrue())
			}
		})
	})
	...
	Context("Homomorphic under addition (2)", func() {
		...
		Specify("summed shares should be a consistent sharing of the sum of the secrets", func() {
			...
			for i := 0; i < trials; i++ {
				...
				Expect(Ed25519SharesAreConsistent(sharesSummed, kmax)).To(BeTrue())
			}
		})
		...
		Specify("adding a constant to shares should result in a consistent sharing of the secrets plus the constant", func() {
			...
			for i := 0; i < trials; i++ {
				...
				Expect(Ed25519SharesAreConsistent(sharesSummed, k)).To(BeTrue())
			}
		})
	})
		...
		Specify(
			...
			func() {
				...
				for i := 0; i < trials; i++ {
					...
					Expect(Ed25519SharesAreConsistent(sharesScaled, k)).To(BeTrue())
				}
			},
		)
	})
...
func TestBenchmarkShare(b *testing.B) {
	n := 100
	k := 33
...
func TestBenchmarkOpen(b *testing.B) {
	n := 100
	k := 33
